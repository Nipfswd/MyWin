...
TemplateName=PSYSTEM_VERIFIER_INFORMATION
NoType=DriverName
IndLevel=0
Direction=OUT
PostCall=
...
TemplateName=PSYSTEM_LOGICAL_PROCESSOR_INFORMATION
NoType=ProcessorMask
NoType=Reserved
IndLevel=0
Direction=OUT
PostCall=
try {
    ((@ArgHostTypeInd *)@ArgHostName)->ProcessorMask = Wow64ThunkAffinityMask64TO32 (@ArgName->ProcessorMask);
    RtlCopyMemory (((@ArgHostTypeInd *)@ArgHostName)->Reserved, @ArgName->Reserved, sizeof (((@ArgHostTypeInd *)@ArgHostName)->Reserved));
...
TemplateName=PPORT_VIEW
Direction=IN OUT
Locals=
End=
PreCall=
if (ARGUMENT_PRESENT (@ArgNameHost)) {
    RetVal = Wow64pThunkLegacyPortViewIn ((PPORT_VIEW32)UlongToPtr (@ArgNameHost), &@ArgName, &LegacyLpcPort);
    if (!NT_SUCCESS (RetVal)) {
        return RetVal;
    }
} else {
    @ArgName = UlongToPtr (@ArgNameHost);
}
End=
PostCall=
if (LegacyLpcPort == TRUE) {
    RetVal = Wow64pThunkLegacyPortViewOut (@ArgName, (PPORT_VIEW32)UlongToPtr (@ArgNameHost));
    if (!NT_SUCCESS (RetVal)) {
        return RetVal;
    }
}
...
TemplateName=PREMOTE_PORT_VIEW
Direction=IN OUT
Locals=
End=
PreCall=
if (ARGUMENT_PRESENT (@ArgNameHost)) {
    ...
    RetVal = Wow64pThunkLegacyRemoteViewIn ((PREMOTE_PORT_VIEW32)UlongToPtr (@ArgNameHost), &@ArgName, &LegacyLpcPort);
    if (!NT_SUCCESS (RetVal)) {
        return RetVal;
    }        
} else {
...
PostCall=
if (LegacyLpcPort == TRUE) {
    RetVal = Wow64pThunkLegacyRemoteViewOut (@ArgName, (PREMOTE_PORT_VIEW32)UlongToPtr (@ArgNameHost));
    if (!NT_SUCCESS (RetVal)) {
        return RetVal;
    }
}    
...
Direction=OUT
Locals=
REMOTE_PORT_VIEW RemotePortViewCopy;
End=
PreCall=
if (ARGUMENT_PRESENT (@ArgNameHost)) {

    if (((PREMOTE_PORT_VIEW)UlongToPtr (@ArgNameHost))->Length == sizeof (REMOTE_PORT_VIEW32)) {
        ...
        RemotePortViewCopy.ViewBase = NULL;
        RemotePortViewCopy.ViewSize = 0;
        @ArgName = &RemotePortViewCopy;
...
Locals=
    ...
    // Legacy lpc support is implemented in each API.
    //
End=
PreCall=
